...
class OverlayOptionsTest {
    ...
    fun brandNormalize() {
        ...
        assertEquals(Brand.SECURED_CONNECT, Brand.normalize("Secured-Connect"))
        // DEFAULT is CONNECT for this SDK → unknown/null/empty normalize to it.
        assertEquals(Brand.CONNECT, Brand.normalize("unknown"))
        assertEquals(Brand.CONNECT, Brand.normalize(null))
        assertEquals(Brand.CONNECT, Brand.normalize(""))
        ...
        assertEquals(Brand.CONNECT, Brand.normalize("securedConnect"))
        assertEquals(Brand.CONNECT, Brand.normalize("SECURED_CONNECT"))
    }
