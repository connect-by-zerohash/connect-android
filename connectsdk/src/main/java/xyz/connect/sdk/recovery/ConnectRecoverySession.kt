...
/**
 * Manages the lifecycle of a recovery session.
 *
 * Responsible for:
 ...
 */
class ConnectRecoverySession internal constructor(
    ...
    companion object {
        private const val TAG = "ConnectRecoverySession"
        private const val PATH = "/mobile/#recovery"
    }

    /**
     * Present the recovery session.
     *
    ...
    fun present(activity: Activity): ConnectSession? {
        ...
        val newSession = ConnectSession(app = ConnectApp.RECOVERY)
        session = newSession
        ...
        val callbackHandler = RecoveryCallbackHandler(callbacks)
        ...
        try {
            ...
            callbacks.onError(
                ConnectError.UnknownError("Failed to open Recovery: ${e.message}")
            )
        }
