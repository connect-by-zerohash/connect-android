 ...
 *
 * Never exported. OAuth callbacks reach it via [OAuthCallbackActivity], which
 * is the sole exported surface in the SDK.
 */
class WebViewActivity : AppCompatActivity(),
    ...
    companion object {
        ...
        // F-013: ConcurrentHashMap for thread-safe handler access.
        // F-010 (review): entries are timestamped so that a registered handler
        // that is never consumed (e.g. activity-start failed silently, process
        // killed before onCreate) is evicted on the next setCallbackHandler call.
        ...
        /** F-013: Remove stale handler (called on activity-start failure). */
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        try {
            ...
            val theme = intent.getStringExtra(EXTRA_THEME) ?: "system"
            ...
            if (!allowHosts.isNullOrEmpty()) {
                allowList = ConnectAllowList(allowHosts)
            }
            val webHost = intent.getStringExtra(EXTRA_WEB_HOST) ?: "sdk.connect.xyz"
    ...
    private fun setupUI(url: String, jwt: String, environment: String, theme: String, webHost: String) {
        try {
            ...
            // Pre-seed the cached URL so the legacy @JavascriptInterface
            // origin check passes for messages that arrive during initial load.
    ...
    private fun configureWebView() {
        webView.settings.apply {
            ...
            // F-004 hardening: these defaults differ across API levels —
            // notably allowUniversalAccessFromFileURLs defaults to TRUE on
            // pre-API-30 devices, which is dangerous. Set explicitly.
        ...
        // Per-frame origin filtering via WebMessageListener when supported —
        // a cross-origin iframe inside the loaded page cannot reach the bridge.
        // Fall back to @JavascriptInterface (top-frame URL check) otherwise.
        // Use messageHandler.targetOrigin so sandbox sessions point at
        // sdk.sandbox.connect.xyz instead of the hardcoded production constant.
        ...
        if (WebViewFeature.isFeatureSupported(WebViewFeature.WEB_MESSAGE_LISTENER)) {
            WebViewCompat.addWebMessageListener(
                ...
                if (!isMainFrame) {
                    Log.w(TAG, "Bridge message from subframe rejected")
        ...
        webView.webViewClient = object : WebViewClient() {
            ...
            /**
             * Network-level allow-list enforcement — Android equivalent of
             * iOS SDK's WKContentRuleList.
             *
             * Blocks every sub-resource request (scripts, XHR, fetch, images,
             * WebSockets) whose host is not in [allowList].  Top-level
             * navigation is already fully blocked by [shouldOverrideUrlLoading]
             * above, so this catches programmatic resource loads initiated by
             * the page JS that would bypass a navigation-only check.
             */
    ...
    override fun onDestroy() {
        ...
        try {
            ...
            // Always remove any stale handler even if WebView teardown threw.
