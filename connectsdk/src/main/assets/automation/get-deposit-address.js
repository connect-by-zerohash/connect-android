...
(function () {
  ...
  // The tray click-through is DELETED rather than kept as a fallback, for the same
  // reason as the send flow: it would let a Coinbase change that stops /receive
  // mounting route silently onto a path built from a generated-class div with no
  // testid. Fails loudly instead, naming what it waited for.
  async function awaitReceiveEntry() {
    // Keeps sweeping interstitials while it waits — those can cover the step on a
    // deep landing just as they could on the dashboard, so the sweep is not part of
    // the click-through that was removed.
