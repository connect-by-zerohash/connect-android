...
(function () {
  ...
  //
  // The click-through openers are DELETED rather than kept as a fallback: a fallback
  // that never runs is dead code on a money path, and one that does run hides the
  // regression it was meant to survive. If /send stops mounting the recipient step,
  // this must report that, not quietly drive the flaky dashboard route instead.
  async function enterSendFlow() {
    // Races recipient / "Review pending transfer" / funds-on-hold, so a blocking
    // state is recognised immediately on the deep-landed page. Blocking states are
    // real answers about the account and propagate as themselves.
  ...
  // container mounts before its content, and deciding on that frame is a misread.
  //
  // RISK_SCAM_INTRO is one of them: Coinbase blocks on that screen, and while it
  // was missing here the gate was never reported at all — the poll burned its full
  // 10s budget and answered "processing", so the page was never presented and the
  // user could not see what the transfer was stuck on. It reports as
  // id-verification, which is the wrong copy; it needs its own kind on the web.
  ...
  // Prefer a typed-code method the host can relay over a passkey. SMS before TOTP,
  // matching auth-choose-2fa-method.js in login (which picks PASSWORD, then SMS,
  // then any other non-passkey factor) so a user meets the same factor in both
  // flows. Withdraw has no PASSWORD case; Coinbase does not offer it here.
  //
  // Clicks via humanClick like every other control in this file. realisticClick
  // dispatches pointerdown/pointerup and then calls .click(), so this is a superset
  // of the bare click it replaces.
  //
  // Returns true if it switched to an OTP method.
  ...
  async function enterOtp(code) {
    ...
    // detectAndHandle2fa already tried to pick a method, but it runs during
    // withdraw.start with the page not presented, so don't depend on that click
    // having landed — otherwise waitForElement below burns 15s and throws
    // element_not_found on a screen that never had a field (AUTH-4245).
    //
    // Only click when the field is genuinely absent: re-clicking a method while the
    // code screen is already up would navigate away from it. iOS parity.
    ...
    try {
      ...
      // own name — element_not_found:#one-time-code reads as a Coinbase layout change
      // and is what made the production report unreadable. Gated on having chosen:
      // otherwise it would point every reader at the wrong problem.
      if (choseMethod) {
